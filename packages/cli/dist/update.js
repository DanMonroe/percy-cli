...
import { getPackageJSON } from '@addepar/percy-cli-command/utils'; // filepath where the cache will be read and written to
...
} // Fetch and return release information for @addepar/percy-cli.


async function fetchReleases(pkg) {
  let {
    request
  } = await import('@addepar/percy-client/utils'); // fetch releases from the github api without retries
...
export async function checkForUpdate() {
  ...
  try {
    ...
    if (age !== 0) {
      ...
      log.stderr.write('\n');
      log.warn(`${age > 0 && age < 10 ? 'A new version of @addepar/percy-cli is available!' : 'Heads up! The current version of @addepar/percy-cli is more than 10 releases behind!'} ${range}`);
      log.stderr.write('\n');
    }
  } catch (err) {
